...
export function validateVariable(variable: VariableCreate, index: number): void {
  ...
  if (variable.required && (variable.defaultValue === undefined || variable.defaultValue === "")) {
    throw new VariableValidationError(
      `${prefix}: required variable must have a default value`,
      "defaultValue",
    );
  }
  ...
  if (
    ...
    if (Number.isNaN(val) || !Number.isInteger(val)) {
      throw new VariableValidationError(
        `${prefix}: default value must be an integer`,
        "defaultValue",
      );
    }
    if (variable.minValue !== undefined && variable.minValue !== null && val < variable.minValue) {
      throw new VariableValidationError(
        `${prefix}: default value ${val} is below minimum ${variable.minValue}`,
        "defaultValue",
      );
    }
    if (variable.maxValue !== undefined && variable.maxValue !== null && val > variable.maxValue) {
      throw new VariableValidationError(
        `${prefix}: default value ${val} exceeds maximum ${variable.maxValue}`,
        "defaultValue",
      );
    }
  }
  ...
  if (
    ...
    if (!["true", "false", "1", "0"].includes(String(variable.defaultValue).toLowerCase())) {
      throw new VariableValidationError(
        `${prefix}: boolean default must be true/false`,
        "defaultValue",
      );
    }
  }
  ...
  if (variable.dataType === "select") {
    if (!variable.allowedValues || variable.allowedValues.length === 0) {
      throw new VariableValidationError(
        `${prefix}: select type requires allowedValues`,
        "allowedValues",
      );
    }
    if (
      variable.defaultValue !== undefined &&
      variable.defaultValue !== "" &&
      !variable.allowedValues.includes(String(variable.defaultValue))
    ) {
      throw new VariableValidationError(
        `${prefix}: default value must be one of allowedValues`,
        "defaultValue",
      );
    }
  }
  ...
  if (
    ...
    if (
      ...
      variable.minLength !== null &&
      String(variable.defaultValue).length < variable.minLength
    ) {
      throw new VariableValidationError(
        `${prefix}: default value length is below minimum ${variable.minLength}`,
        "defaultValue",
      );
    }
    if (
      variable.maxLength !== undefined &&
      variable.maxLength !== null &&
      String(variable.defaultValue).length > variable.maxLength
    ) {
      throw new VariableValidationError(
        `${prefix}: default value length exceeds maximum ${variable.maxLength}`,
        "defaultValue",
      );
    }
    if (variable.regexPattern) {
      try {
        ...
        if (!regex.test(String(variable.defaultValue))) {
          throw new VariableValidationError(
            `${prefix}: default value does not match regex pattern`,
            "defaultValue",
          );
        }
      } catch (err) {
...
export function validateVariables(variables: VariableCreate[]): void {
  ...
  for (let i = 0; i < variables.length; i++) {
    ...
    if (envVars.has(v.envVar)) {
      throw new VariableValidationError(
        `Variable[${i}] (${v.name}): duplicate envVar "${v.envVar}"`,
        "envVar",
      );
    }
