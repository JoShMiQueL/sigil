...
curl -s -X POST http://localhost:3000/api/groups \
  ...
  -d '{"name":"Minecraft","description":"Java and Bedrock"}'
...
# Edit group
curl -s -X PATCH http://localhost:3000/api/groups/<id> \
  -H "Cookie: sigil_session=<admin-session>" \
  -H "Content-Type: application/json" \
  -d '{"name":"Minecraft Java"}'

# Delete empty group
curl -s -X DELETE http://localhost:3000/api/groups/<id> \
  -H "Cookie: sigil_session=<admin-session>"

# Try to delete non-empty group → 409
```

**Expected**: Group created, listed, edited, deleted. Non-empty group deletion returns 409.
...
# Import the exported YAML into a different group
curl -s -X POST http://localhost:3000/api/templates/import \
  -H "Cookie: sigil_session=<admin-session>" \
  -F "file=@paper-mc.yaml" \
  -F "groupId=<other-group-id>" \
  -F "conflict=skip"
```
...
3. Create a group "Minecraft" through the UI
4. Create a template "Paper MC" through the UI with a variable
5. Activate the template — verify it shows as active
6. Verify the template appears in the user-facing template list (switch to user view or check via SSE)
7. Deactivate the template — verify it disappears from user view
8. Import a PTDL_v2 egg file — verify the import dialog shows skipped fields
...
12. Install a template from the registry — verify it appears as inactive in Installed templates

**Expected**: All UI flows work without page reloads. SSE updates reflect template/group changes in real-time.
