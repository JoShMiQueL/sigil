...
| tags | text[] | NOT NULL, default '{}' | Free-form categorization tags (e.g., {"minecraft","java"}). Multiple tags per template. Source of truth: registry index entry for registry-installed templates; user-defined for local templates. |
...
**Unique constraint**: `name` — template names unique across the panel.

**Index**: GIN index on `tags` for efficient tag filtering (`CREATE INDEX templates_tags_idx ON templates USING GIN (tags)`).
...
│ id       │       │ id       │
│ url      │       │ tags[]   │
│ name     │       │ registryId┼─FK──▶ Registry.id (nullable)
│ authMethod│      │ sourceId │
│ token    │       │ sourceHash│
│ username │       │ name     │
│ password │       │ desc     │
...
Update `packages/db/src/schema/index.ts` to export all three.

## Migration

A single Drizzle migration creates all three tables with proper FK constraints, the unique index on `templates.name`, and the GIN index on `templates.tags`.
